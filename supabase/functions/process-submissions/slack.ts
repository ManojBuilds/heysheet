export async function createFormSubmissionMessage(data) {
  const formattedData = Object.entries(data.submission.data).map(([key, value])=>`• *${key}:* ${value}`).join('\n');
  return {
    blocks: [
      {
        type: "header",
...
export async function sendMessage(channel, message, slackToken) {
